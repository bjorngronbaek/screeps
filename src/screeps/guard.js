 ...
 */
 ...
 if(DEBUG) {console.log('[GUARD] '+creep.name+': '+message);}
 }
 ...
 if(exit){
 var path = creep.pos.findPathTo(exit, {maxOps: 1000, ignoreDestructibleStructures: true, ignoreCreeps : true});
 }
 ...
 var targets = creep.pos.findClosest(FIND_HOSTILE_CREEPS,{filter: function(creep){ return creep.owner.username != 'Source Keeper'}});
 if(!targets){
 targets = creep.pos.findClosest(FIND_HOSTILE_STRUCTURES,{filter:function(s){
